			...
			<view class="tabBar_list" :style="{paddingBottom:is_lhp?'40rpx':''}">
				<view v-for="(item) in tab_nav_list" :key="item.id" :class="{'tabBar_item':item.id!=2,'tabBar_item2':item.id==2}"
				 @tap="cut_index(item.id)">
					<image v-if="show_index == item.id" :src="`../../static/tabBar/${item.id+1}${item.id+1}.`+(show_index == 2?'png':'gif')"></image>
		...
		methods: {
			...
			onPullDownRefresh() {
				...
				let _this = this
					...
					let show_index=this.show_index+1
					setTimeout(function() {
...
<style lang="scss">
	...
	.tabBar_list>view:nth-child(3) image {
